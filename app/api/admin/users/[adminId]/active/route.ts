...
/** POST /api/admin/users/[adminId]/active — activates/deactivates (soft delete) an operator account from the Administradores screen (Sprint ADMIN-1). */

const adminUserRepository = new PrismaAdminUserRepository();
const auditLogRepository = new PrismaAuditLogRepository();

const setAdminActiveUseCase = new SetAdminActiveUseCase(adminUserRepository, auditLogRepository);
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const parsed = setActiveSchema.safeParse(payload);
  if (!parsed.success) {
    return errorResponse(400, "invalid_request", "An `active` boolean is required.");
  }

  try {
    const result = await setAdminActiveUseCase.execute({
      adminId,
...
function handleUseCaseError(error: unknown, adminId: string): NextResponse {
  ...
  logger.error("Unexpected error while changing an admin user's active state", {
    adminId,
