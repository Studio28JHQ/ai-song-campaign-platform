...
- **Resume link now returns to the actual step left off at**: `GET /api/leads/session` reported only `approvedLyrics`, so a parent who generated a lyrics version but never approved it came back through the emailed "Ver el progreso de mi canción" link to a blank generation form — their pending version still in the database but unreachable from the UI — and the next submit was a _regeneration_ server-side, silently spending one of their attempts to re-create what they already had. The session state now also carries `pendingLyrics`, and `LyricsWorkflow` restores the review panel for that exact version. Defined as _the newest version, and only while it is still unapproved_: a version left unapproved behind an approved one is not pending (approval is terminal — `GenerateLyricsForLeadUseCase` refuses to generate past it), while a version created after an approval is still the newest thing the lead has; `approvedLyrics` is checked first when rendering either way, so an approved version is never displaced. Contradicted the welcome email's own promise ("este mismo enlace te llevará siempre al paso en el que te quedaste" — `WelcomeEmailTemplate`). No migration: the data already existed, only the read changed.
- **"Quiero otra versión" keeps working identically on a restored version**: restoring a pending version made the review panel's regenerate button reachable in a state that previously could not occur, with no request to repeat. `LyricsWorkflow` now rebuilds the original request from the three fields `pendingLyrics` carries (`moodId`, `parentMessage`, `voice` — all persisted on `Lyrics`, all the parent's own input), resolving `moodName`/`moodDescription` through `resolveMood`, extracted verbatim from `LyricsGenerationForm`'s own submit so both paths share one mood mapping instead of two that can drift. `handleGenerateAgain` itself is unchanged: regeneration after a resume sends the same mood, message, and voice the uninterrupted flow would have. The Claude-authored `prompt`/`musicMood`/`musicDirection` stay server-side — the parent never needs them, so they are never sent. A version generated before `parentMessage` was persisted (nullable for rows predating Sprint v1.1) can still be shown and approved, but cannot be regenerated from, since the message it came from was never stored.
- **Turnstile is correctly omitted, never emptied, on a restored regeneration**: `POST /api/lyrics/generate` verifies Turnstile only on a lead's _first_ generation, and its schema accepts `turnstileToken` as optional but rejects an empty string with a `400`. A rebuilt request has no token to send, so the field is now optional on `LyricsGenerationSubmitValues`/`GenerateLyricsInput` and left `undefined` — `JSON.stringify` drops it entirely rather than sending `""`. The form still requires a token client-side for a first generation, and the route's own first-generation requirement is untouched.
- **Welcome-email failures are now diagnosable**: the `after()` handler in `POST /api/leads` logged only `error.message`, discarding the `code`/`context` `ResendClient` attaches (the HTTP status and Resend's parsed error body) — which made a provider rejection indistinguishable from any other delivery failure, in particular an unverified sender domain, which Resend rejects with a `403` and an explanatory body rather than a bounce. Both are now logged, mirroring `logDiagnostics` in `app/api/lyrics/generate/route.ts`. Registration still never waits on, or fails because of, email delivery — this only widens the log.

### Changed

- **`LYRICS_TOO_LONG_RETRY_LIMIT` raised from 1 to 2** (`ClaudeLyricsService`): a live re-measurement of the current prompt put the single-call overshoot of `ResponseParser`'s 360-character hard maximum at roughly 1 in 8 (one 454-character lyric in 8 consecutive generations), well above the ~2 in 21 observed when the constant was introduced. At that rate a single retry still surfaced "El servicio de creación de letras no está disponible en este momento" (`claude_unavailable`) on roughly 1.5% of generations; a second takes that to roughly 0.2%, for a worst case of 3 Claude calls. Still bounded, and still spent only on the one failure mode a fresh generation actually fixes — every other malformed-response rejection remains un-retried.

### Maintenance

- Added coverage for the resume path across all four pending/approved combinations (no version; one unapproved; an approved newest version; a version created after an approval), for the rebuilt regeneration request (exact body, including the absent `turnstileToken`), for approving a restored version directly, and for the legacy null-`parentMessage` case. Fixed the in-memory `LyricsRepository` fake in `tests/application/lead/GetLeadSessionStateUseCase.test.ts`, which returned insertion order rather than the version-ascending order `PrismaLyricsRepository` documents — an ordering regression would have passed unnoticed.
- `GetLeadSessionStateUseCase` now derives both the approved and the pending version from a single `findAllByLead` read instead of an additional `findApprovedByLead` query, keeping the endpoint at the same two repository calls it made before this release. Safe because the `lyrics_one_approved_per_lead` partial unique index (`ON lyrics("leadId") WHERE approved = true`) guarantees at most one approved version per lead. `LyricsRepository`'s contract is unchanged and `findApprovedByLead` is still used by its other callers.
...
- **Campaign Banner**: a new `CampaignBanner` Landing section (`src/features/landing/components/`) renders the client's campaign artwork (`public/campaign/banners/banner-campaign.jpg`, a new per-kind asset folder alongside `backgrounds/`, `products/`, etc.) between the Hero and Products sections. Built on the same `CampaignSection`/`CampaignContainer` shell every other Landing section uses (same max-width, same vertical rhythm), with `next/image` given the asset's real intrinsic size (1920×1000) so it reserves the correct aspect ratio up front — no layout shift — while scaling fluidly to 100% of the container width (`w-full h-auto`) at every breakpoint. Rounded `24px` to match the existing card/section corner radius.
...
- **Removed consent verification test data**: live end-to-end verification of the above (accepting the banner twice, exporting the CSV once) left 2 `Consent` rows and 1 `export_consents` `AuditLog` entry in the database — no code change, database only. Identified unambiguously as test artifacts (both `Consent` rows had `ipAddress: "::1"` — no real campaign visitor ever connects from localhost — and matched the exact timestamps/session of the verification steps; the `AuditLog` entry matched the exact admin id and timestamp used only for that same test) and removed; confirmed zero rows remain matching either criterion afterward. No production data existed to affect — these were the only rows in either table at the time.

## [1.29.0] - 2026-07-24

### Added

- **Google Tag Manager Configuration**: a new, database-backed global campaign setting — `Campaign.gtmContainerId` — lets the campaign team configure a GTM container id from the Admin panel (Configuración → "Google Tag Manager"), with no code change or redeploy required. When set, the Landing renders Google's official GTM snippet (`next/script` + `<noscript>` iframe fallback, served by a new client component, `GoogleTagManager`, reading a new public `GET /api/settings/gtm` endpoint); when empty, no tracking code of any kind is rendered anywhere in the application. `UpdateGtmSettingsUseCase` (`PATCH /api/admin/settings`, admin-session-gated) validates the id against `GTM-[A-Z0-9]+` before persisting and writes an audit log entry, mirroring every other admin mutation. No environment variable, no hardcoded id, no other analytics provider integrated directly — all future tracking must be configured inside GTM itself. Implemented via a new narrow port, `CampaignSettingsGate` (`src/application/campaign/`), the same "thin Prisma adapter, no full aggregate" pattern already used for `CampaignGate` — see `docs/Architecture/System_Architecture.md` and `docs/Architecture/External_Services.md`.
- **Privacy Consent Module**: the Landing now shows a cookie/privacy consent banner (`ConsentBanner`, built entirely from the existing `Campaign*` design-system components) until the visitor accepts it, then never again (tracked via `localStorage`). Accepting it (`POST /api/consent`, public) generates a first-party `consent_session_id` cookie (if none exists yet) and creates exactly one anonymous `Consent` record per session — `sessionId`, `ipAddress`, `userAgent`, `policyVersion`, `acceptedAt` — never duplicated for a repeat accept (enforced by both `RecordConsentUseCase` and a unique database constraint on `sessionId`). If the same visitor later completes registration, `POST /api/leads` associates the existing `Consent` with the new `Lead` via that same session id — a best-effort step that can never fail or block registration, and that only ever updates the existing record, never creates a second one. New `consent` domain/application/infrastructure module (`src/domain/consent/`, `src/application/consent/`, `src/infrastructure/persistence/prisma/consent/`), layered exactly like `lead`/`lyrics`/`song` — see `docs/Architecture/System_Architecture.md` — Privacy Consent Module and `docs/Product/Business_Rules.md` — Consent Rules.
- **Schema**: new migration `20260724120000_add_campaign_gtm_and_consent` — adds `campaigns.gtmContainerId` (nullable `TEXT`) and a new `consents` table (`sessionId` unique, `leadId` nullable/unique, `FK → leads` `ON DELETE SET NULL`). No existing table, column, or constraint touched. Verified: the migration's SQL is byte-identical to Prisma's own generated diff against the live database before applying it; applied via `prisma migrate deploy`, `prisma generate` regenerated the client, and `prisma migrate status` confirms the schema is up to date.
...
- **Song generation stopped progressing after the first poll returned `pending`, and never resumed on its own**: root cause was twofold. (1) `GenerationDispatcher`/`GenerationPoller` only ever ran once per user-facing request (`/api/lyrics/approve`, `/api/song/generate`, `/api/admin/songs/[songId]/retry`, each duplicating its own dispatcher/poller instantiation) or once per external scheduler tick (`GET /api/internal/pipeline/run`, every 10 minutes, best-effort) — a song whose first poll landed before Mureka finished (the common case; Mureka typically takes 50–90+ seconds) had no other trigger to advance it except unrelated future traffic happening to arrive. (2) Confirmed via `gh run list`/`gh run view` that the external GitHub Actions scheduler has been failing on every single invocation (`HTTP 401 {"error":"unauthorized"}`) since at least 2026-07-17 — the workflow's `CRON_SECRET` resolves empty in that environment (a GitHub repository secret configuration issue, out of this fix's scope — see "Remaining manual configuration" below; not resolved here per instruction not to touch repository secrets).
  Fix: `/api/internal/pipeline/run` now reschedules itself — after each tick, if the dispatcher just claimed a song or the poller found one to poll (i.e. there's still something `GENERATING`, or the queue may have another song waiting), it schedules exactly one more tick ~5 seconds later via `after()`, calling itself through a new `triggerPipelineTick` helper. This self-perpetuating chain continues, entirely in-process, until a tick finds nothing left to do (queue empty and nothing `GENERATING`), which is also the exact condition under which it correctly stops. The three request-triggered call sites no longer instantiate their own `GenerationDispatcher`/`GenerationPoller` at all — they now only call `triggerPipelineTick` to place the chain's first link, so there is exactly one polling implementation in the codebase, not four. `GenerationDispatcher`/`GenerationPoller` themselves, the Mureka/Claude integrations, and the lyrics/approval workflows are unchanged. The external scheduler is unchanged in behavior and remains a periodic safety net on top of this, not the primary mechanism, going forward.
  Live end-to-end verified (no manual `/api/internal/pipeline/run` call, no restart): three leads registered and approved in sequence; the first song was already stuck `GENERATING` under the old code, the second was freshly submitted, the third queued behind both — after the fix, a single new approval's background trigger correctly resumed the stuck song, and all three reached `COMPLETED` with audio persisted to R2 automatically over the following ~2.5 minutes, each with a distinct `providerTaskId` (no duplicate Mureka submissions) and processed strictly in submission order (no skipped queue entries).
  A related bug was caught and fixed during this same verification: `triggerPipelineTick`'s self-call originally used `appConfig.url`/`NEXT_PUBLIC_APP_URL` (the stable public domain used for user-facing email links) — in a local/dev environment (and in any preview deployment) this is a _different_ host than the one actually serving the request, so the self-call silently went to the wrong deployment and never advanced the local queue. Every call site now passes `new URL(request.url).origin` instead, so the self-call always targets whichever host is actually running.
