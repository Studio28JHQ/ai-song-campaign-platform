...
6. **Consentimientos** (Feature 1 — Consent Management Screen) — `/admin/consents` shows the latest 20 `Consent` records (every stored field, plus the associated Lead when one exists, linking to its Lead Detail page) and a "Descargar CSV" action that downloads every Consent record in the database, unfiltered and unpaginated — see docs/Architecture/System_Architecture.md — Privacy Consent Module ("Admin reporting").
7. **Sign out** — a "Log out" button (present on the dashboard) calls `POST /api/admin/logout`, which clears the session cookie.

**Audit history.** Actions are recorded automatically as the operator uses the module: a successful login (`action: "login"`, against the `AdminUser`), viewing a lead's detail page (`action: "view_lead"`, against that `Lead`), exporting the Consentimientos CSV (`action: "export_consents"`, against `Consent`), and the two operational recovery actions below (`retry_song`/`resend_email`, against the `Song`). There is no UI to create, edit, or delete an audit entry — they exist purely as the read-only trail shown on the Lead Detail screen.
