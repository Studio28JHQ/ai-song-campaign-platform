...
A single, DB-backed global setting — `Campaign.gtmContainerId` — controls whether, and with which container id, Google Tag Manager loads on the Landing. There is no Campaign domain aggregate (see `docs/Architecture/Domain_Model.md`), so reads/writes go through a narrow port, `CampaignSettingsGate` (`src/application/campaign/contracts/`), satisfied by `PrismaCampaignSettingsGate` — the same "narrow port over a Prisma adapter" pattern as `CampaignGate` (`src/application/song/contracts/`).

**Admin write path.** `GET`/`PATCH /api/admin/settings` (admin-session-gated, like every other `/api/admin/*` route) wrap `GetCampaignSettingsUseCase`/`UpdateGtmSettingsUseCase` (`src/application/admin/use-cases/`). `UpdateGtmSettingsUseCase` normalizes the input (trims, uppercases, blank → `null`), validates it against `GTM-[A-Z0-9]+` when non-empty, persists it, and writes an `AuditLogEntry` (`action: "update_gtm_settings"`) — the same audit pattern every other admin mutation already follows. The Admin Settings screen (`GtmSettingsForm`, `src/features/admin/components/`) is a small form following the exact `services/`+`hooks/`+`components/` shape every other admin screen uses (see `AdminUsersList`/`useAdminUsersList`).

**Public read path.** The Landing cannot use the admin-gated route, so a second, public, read-only endpoint exists — `GET /api/settings/gtm` — reusing `GetCampaignSettingsUseCase` unchanged. `GoogleTagManager` (`src/features/landing/components/`) is a client component (matching this codebase's established convention of client-side data fetching for every dynamic read — see `docs/Architecture/Folder_Structure.md`, `features/*/services`) that fetches this endpoint on mount and, only when a container id is present, renders Google's official snippet via `next/script` (`strategy="afterInteractive"`) plus a `<noscript>` iframe fallback — nothing is ever rendered when the setting is empty. A failed/slow lookup degrades silently to "no GTM code," never a visible error.

## Privacy Consent Module

Feature 2 adds a fourth parent-facing-adjacent module, `consent` (`src/domain/consent/`, `src/application/consent/`, `src/infrastructure/persistence/prisma/consent/`), layered exactly like `lead`/`lyrics`/`song` — no new architectural pattern.

**Domain.** `Consent` (`src/domain/consent/entities/`) is a small aggregate root: `sessionId`, an initially-`null` `leadId`, `ipAddress`, `userAgent`, `policyVersion`, and `acceptedAt`. Its only behavior beyond construction is `associateWithLead(leadId)` — idempotent if called again with the same lead, and a `BusinessRuleError` if called with a _different_ one, so a Consent can never silently change owners. `ConsentRepository` (`src/domain/consent/repositories/`) is the persistence contract: `findBySessionId`, `create`, `update` — no `delete`, since a Consent is never removed by application code.

**First-party session.** A dedicated cookie, `consent_session_id` (`src/infrastructure/consent/consentSessionCookie.ts` — `HttpOnly`, `Secure`, `SameSite=Lax`, 30 days, mirroring `leadSessionCookie.ts`), identifies the visitor before any Lead exists. `POST /api/consent` (public, no auth) is the only place this cookie is created: if the incoming request has none, a new random UUID is generated. `RecordConsentUseCase` then either returns the existing `Consent` for that session unchanged (idempotent — "exactly one Consent per session, never duplicated," backstopped by the `sessionId` unique constraint at the database level, see `docs/Architecture/Database_Model.md`) or creates a new one.

**Associating with a Lead.** `POST /api/leads` (`app/api/leads/route.ts`), immediately after a successful `CreateLeadUseCase.execute`, reads the `consent_session_id` cookie (if any) and calls `AssociateConsentWithLeadUseCase` — a best-effort, independently try/caught step that can never fail registration itself, the same tolerance already applied to the welcome-email send in the same route. If no cookie or no matching `Consent` exists (the visitor registered without ever accepting the banner), this is a silent no-op — consent is not a prerequisite for registration, only something to link when it already exists. The association is always performed by `sessionId`, never creates a second `Consent`, and only updates the existing record's `leadId`.

**Landing UI.** `ConsentBanner` (`src/features/consent/components/`) is a client component shown only until accepted — tracked via a `localStorage` flag so a returning visitor never sees it again without a network round trip — built from the existing `Campaign*` design-system components (`CampaignCard`/`CampaignButton`), matching the Landing's established visual language with no new styling primitives. Accepting calls `POST /api/consent`. It deliberately skips `CampaignContainer` (the max-width shell every other Landing section shares): the banner instead sizes itself to `w-[90vw]`, centered, a look specific to this banner alone — see "Consent Banner UI" below. Because it renders as a sibling of `<main>` in `app/page.tsx` (so it can overlay the footer, not just the content column), it carries its own `theme-campaign campaign-landing` classes directly, rather than inheriting them from `<main>` — without them, its colors and body font would fall back to the unrelated default (non-campaign) theme.

**Consent Banner UI.** The Accept button overrides only its own `bg-*`/`hover:bg-*` Tailwind utilities via `className` (never `CampaignButton` itself, which every other primary action on the Landing — e.g. the registration form's submit button — still uses unmodified): `bg-primary/80` by default (a lighter tint of the brand purple, `#8B5CF6` at 80% opacity) transitioning to `bg-primary` (the exact brand color, opaque) on hover, with `text-primary-foreground` (white) preserved. No new CSS file and no new color token — both states are the same existing `--primary` token, just at different opacities, exactly the "prefer existing tokens over new ones" rule in `docs/Architecture/Design_System.md`.

**Admin reporting.** The "Consentimientos" screen (`/admin/consents`, `ConsentsList` — `src/features/admin/components/`) shows the latest 20 `Consent` records, every stored field, plus the associated Lead (parent/baby name, linking to `/admin/leads/{leadId}`) read through the existing `Consent → Lead` relationship — no additional persistence. Like every other admin reporting screen (Dashboard, Leads, Lyrics), reads have no natural home on the domain `ConsentRepository` (scoped to the Consent aggregate's own use cases — lookup by session, create, update), so a narrow, admin-specific read-model port, `AdminConsentGate` (`src/application/admin/contracts/`), backs both the on-screen list (`listLatest`) and the CSV export (`streamAll`) — the same "narrow port over a Prisma adapter" pattern as `AdminLyricsListGate`/`AdminLeadExportGate`. `GET /api/admin/consents/export` streams **every** Consent row (unfiltered, unpaginated per this feature's requirements) as CSV, batched via keyset (cursor) pagination rather than `skip`/`take` — unlike the Leads export (bounded by the campaign's ≤3,000-lead cap), Consent rows accumulate from every Landing visit, not just registrations, so this keeps each batch query's cost independent of how deep into the export it is. Every export writes an `export_consents` audit entry first (PII — `ipAddress`/`userAgent` — leaves the system on every export), the same reasoning `export_leads` already follows; both routes now share one CSV-serialization helper, `src/shared/utils/csv.ts` (`csvEscape`/`toCsvLine`, including the same formula-injection guard), rather than duplicating it.
