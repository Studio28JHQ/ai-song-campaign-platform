...
**Landing UI.** `ConsentBanner` (`src/features/consent/components/`) is a client component shown only until accepted — tracked via a `localStorage` flag so a returning visitor never sees it again without a network round trip — built entirely from the existing `Campaign*` design-system components (`CampaignCard`/`CampaignButton`/`CampaignContainer`), matching the Landing's established visual language with no new styling primitives. Accepting calls `POST /api/consent`.
