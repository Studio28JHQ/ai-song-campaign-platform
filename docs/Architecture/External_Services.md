...
- **`PromptBuilder`** — builds the request payload from the approved lyrics text and the Mood's fixed prompt; pins `n: 1` to enforce "exactly one song per call" (see `docs/Product/Business_Rules.md` — Song Rules) and `model: "auto"` per Mureka's own quickstart example.
